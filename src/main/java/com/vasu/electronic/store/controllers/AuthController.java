...
import com.vasu.electronic.store.dtos.JwtRequest;
import com.vasu.electronic.store.dtos.JwtResponse;
...
import com.vasu.electronic.store.security.JwtHelper;
...
public class AuthController {
    ...
    public ResponseEntity<JwtResponse> loginWithGoogle(@RequestBody Map<String, Object> data) throws IOException {
        ...
        try {
            googleIdToken = verifier.build().verify(idToken);
        } catch (Exception e) {
            throw new BadApiRequestException("Invalid Google ID Token: " + e.getMessage());
        }

        if (googleIdToken == null) {
            throw new BadApiRequestException("Invalid Google ID Token");
        }
        ...
        if (user == null) {
            ...
            user = this.saveUser(email, name, photoUrl != null ? photoUrl : "");
        }
